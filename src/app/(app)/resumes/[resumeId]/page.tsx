...
export default async function ResumeEditorPage({ params }: {params:Promise<{resumeId:string}>}) {
  ...
  if (!resume) notFound();
  return (<ResumeBuilder resume={{...resume, updatedAt: resume.updatedAt.toISOString()}} templates={getResumeTemplates()} canUsePremiumTemplates={canUsePremiumTemplates(user.subscriptionTier)} canUseAiWriting={canUseAiWriting(user.subscriptionTier)} />);
}
